...
const sessionUserColumns = publicColumns
  .split(',')
  .map((column) => `u.${column}`)
  .join(',');
...
export class AuthService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    await this.db.transaction(async () => {
      ...
      if (await this.db.get("SELECT id FROM users WHERE role = 'ADMIN'")) return;
  ...
  throttle(key: string): void {
    const now = Date.now();
    for (const [entry, value] of this.attempts) if (value.until <= now) this.attempts.delete(entry);
    ...
    if (state.count > 15)
      throw new HttpException('Too many attempts. Please try again in 15 minutes.', 429);
  }
  ...
  async authenticate(token: string | undefined): Promise<User> {
    ...
    const user = (await this.authenticateMany([token])).get(token);
    if (!user) throw new UnauthorizedException('Your session expired. Please sign in again');
    return user;
  }

  /** Fresh session lookup for a bounded batch; revoked/invalid tokens are omitted. */
  async authenticateMany(tokens: readonly unknown[]): Promise<Map<string, User>> {
    const tokensByHash = new Map<string, string>();
    for (const token of tokens) {
      if (typeof token === 'string' && SESSION_TOKEN_PATTERN.test(token))
        tokensByHash.set(this.digest(token), token);
    }
    if (!tokensByHash.size) return new Map();
    const rows = await this.db.all<User & { tokenHash: string }>(
      `SELECT ${sessionUserColumns},s."tokenHash" FROM users u
       JOIN sessions s ON s."userId"=u.id
       WHERE s."tokenHash" = ANY($1::text[]) AND s."expiresAt" > $2`,
      [...tokensByHash.keys()],
      new Date().toISOString(),
    );
    const users = new Map<string, User>();
    for (const { tokenHash, ...user } of rows) {
      const token = tokensByHash.get(tokenHash);
      if (token) users.set(token, user);
    }
    return users;
  }
  ...
  async logout(token: string): Promise<void> {
    await this.db.run('DELETE FROM sessions WHERE "tokenHash" = $1', this.digest(token));
  }
  ...
  private async issueSession(id: string) {
    ...
    await this.db.transaction(async () => {
      await this.db.run('DELETE FROM sessions WHERE "expiresAt" <= $1', new Date().toISOString());
      await this.db.run(
    ...
    return {
      token,
      expiresAt,
      user: (await this.db.get<User>(`SELECT ${publicColumns} FROM users WHERE id = $1`, id))!,
    };
  }
