...
export class VehiclesService {
  ...
  async findOne(id: string): Promise<Vehicle> {
    const vehicle = await this.db.get<Vehicle>('SELECT * FROM vehicles WHERE id = $1', id);
  ...
  async update(id: string, input: UpdateVehicleDto): Promise<Vehicle> {
    return this.write(async () => {
      ...
      if (input.imei && input.imei !== existing.imei) await this.assertImeiAvailable(input.imei);
      await this.db.run(
  ...
  async remove(id: string): Promise<void> {
    return this.write(async () => {
      ...
      if (await this.db.get('SELECT id FROM routes WHERE "vehicleId" = $1', id))
        throw new ConflictException('This vehicle is assigned to a route and cannot be deleted');
  ...
  private async saveDetails(id: string, input: CreateVehicleDto | UpdateVehicleDto): Promise<void> {
    const fields = (
      ['model', 'purchaseDate', 'fitnessExpiresAt', 'licenseExpiresAt', 'status'] as const
    ).filter((field) => input[field] !== undefined && input[field] !== null);
    if (!fields.length) return;
    // Identifiers are from the fixed allowlist above; all values are parameters.
    const assignments = fields.map((field, index) => `"${field}"=$${index + 1}`);
    await this.db.run(
      `UPDATE vehicles SET ${assignments.join(',')} WHERE id=$${fields.length + 1}`,
  ...
  private async syncDriver(id: string, input: CreateVehicleDto | UpdateVehicleDto): Promise<void> {
    ...
    const assigned = await this.db.get<{ id: string }>(
      'SELECT id FROM drivers WHERE "vehicleId"=$1',
      id,
    );
    if (assigned) {
      await this.db.run(
        'UPDATE drivers SET name=$1,phone=$2 WHERE id=$3',
        vehicle.driverName ?? '',
        vehicle.driverPhone ?? '',
        assigned.id,
      );
