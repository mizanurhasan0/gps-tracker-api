...
export function parseGpsTime(value: string, offsetMinutes = 0): string | null {
  ...
  if (
    ...
    return null;
  return new Date(
    date.getTime() - (match[7] ? 0 : offsetMinutes * 60_000),
  ).toISOString();
}
...
export function validateRange(
  ...
  if (!/^\d{10,20}$/.test(imei))
    throw new BadRequestException('Invalid device IMEI');
...
export function disconnected(a: HistoryPoint, b: HistoryPoint): boolean {
...
export function buildRoute(points: HistoryPoint[], maxPoints: number) {
  ...
  const displayedPointCount = displayed.reduce(
...
export function sampleSegments(
  ...
  const mandatory = segments.reduce(
    (sum, s) => sum + Math.min(2, s.points.length),
    0,
  );
  ...
  return segments.map(segment => {
    ...
    const take = remainingInterior
      ? Math.min(
          available,
          Math.floor((budget * available) / remainingInterior),
        )
      : 0;
    ...
    for (let i = 1; i <= take; i++)
      result.push(
        segment.points[
          Math.floor((i * (segment.points.length - 1)) / (take + 1))
        ],
      );
    if (segment.points.length > 1)
      result.push(segment.points[segment.points.length - 1]);
...
export function summarize(points: HistoryPoint[], from: string, to: string) {
  ...
  for (const point of points) {
    const date = new Date(Date.parse(point.gpsTime) + DHAKA_OFFSET)
      .toISOString()
      .slice(0, 10);
