...
test('student shifts, weekly travel and cross-table concurrency HTTP integration', { skip: !databaseUrl }, async t => {
  ...
  await t.test('overlaps warn, profile identities survive stopping, and bills distinguish each shift', async () => {
    ...
    const futureDate = new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Dhaka',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(Date.now()+86400000));
    const priorMonthDate = new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Dhaka',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(Date.now()-40*86400000));
    await request(`/admin/students/${day.id}/stop`, admin.token, { stopDate: futureDate, finalMonthlyFee: 100000 }, 'PATCH', 400);
    await request(`/admin/students/${day.id}/stop`, admin.token, { stopDate: priorMonthDate, finalMonthlyFee: 100000 }, 'PATCH', 400);
    const morningBill = own.find((bill: Row) => bill.subscriptionId === morning.id);
    await db.run(`UPDATE bills SET status='PAID',"paidAt"=$1 WHERE id=$2`, new Date().toISOString(), morningBill.id);
    await request(`/admin/students/${morning.id}/stop`, admin.token, { stopDate, finalMonthlyFee: morningBill.amount - 1 }, 'PATCH', 409);
    await db.run(`UPDATE bills SET status='UNPAID',"paidAt"=NULL WHERE id=$1`, morningBill.id);
    const submissionId = randomUUID();
    await db.run(`INSERT INTO payment_submissions(id,"billId","guardianId",method,"recipientNumber","senderNumber","transactionId",amount,status,"createdAt") VALUES($1,$2,$3,'BKASH','01700000001','01700000002',$4,$5,'PENDING',$6)`, submissionId, morningBill.id, guardian.user.id, `pending-${submissionId}`, morningBill.amount, new Date().toISOString());
    await request(`/admin/students/${morning.id}/stop`, admin.token, { stopDate, finalMonthlyFee: morningBill.amount }, 'PATCH', 409);
    await db.run('DELETE FROM payment_submissions WHERE id=$1', submissionId);
    const attendanceBeforeStop = (await db.get('SELECT count(*)::int count FROM attendance WHERE "studentId"=$1',day.id)).count;
    const stopped = await request(`/admin/students/${day.id}/stop`, admin.token, { stopDate, finalMonthlyFee: 100000, reason: 'Stopped after a few days' }, 'PATCH');
    assert.equal(stopped.settlement.billAction,'ADJUSTED');
    assert.equal(stopped.finalBill.amount,100000);
    assert.equal((await db.get('SELECT count(*)::int count FROM attendance WHERE "studentId"=$1',day.id)).count,attendanceBeforeStop);
  ...
  await t.test('archive hides a canonical student, stops every shift, preserves history, and restore does not reactivate service', async () => {
    ...
    if (!pending) {
      ...
      assert.ok(evening);
      const stopDate = new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Dhaka',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
      await request(`/admin/students/${evening.id}/stop`, admin.token, { stopDate, finalMonthlyFee: 200000 }, 'PATCH');
    ...
    const pendingStop = await request('/stop-requests', guardian.token, { subscriptionId: morning.id, reason: 'Archive requested' }, 'POST', 201);
    ...
    assert.equal(resolvedStop.status, 'APPROVED');
    assert.match(resolvedStop.note, /archiving/);
    assert.ok(resolvedStop.reviewedAt);
