...
test(
  ...
  async (t) => {
    ...
    try {
      ...
      await t.test('protects endpoints and rejects role injection and invalid login', async () => {
        ...
        const vehicles = await request<{ vehicles: { id: string }[] }>('/vehicles', admin.token);
      ...
      await t.test(
        ...
        async () => {
          ...
          assert.equal(
            Number(
              (await db.get('SELECT count(*) AS total FROM users WHERE phone = $1', input.phone))
                .total,
            ),
            1,
          );
          ...
          await request('/admin/requests', renewed.token, undefined, 'GET', 403);
        },
      );
      ...
      await t.test('validates route coverage and pending requests', async () => {
        ...
        await request('/locations/868720065798377', guardian.token, undefined, 'GET', 403);
      });
      await request(
      ...
      const subscriptions = await request<Identified[]>('/subscriptions', guardian.token);
      ...
      await t.test('restricts tracking by approved assignment', async () => {
        ...
        const vehicles = await request<{ vehicles: { id: string }[] }>('/vehicles', guardian.token);
        assert.equal(vehicles.vehicles.length, 1);
        await request('/locations/868720065798377', guardian.token);
        await request('/locations/868720065798377', other.token, undefined, 'GET', 403);
        await request(`/vehicles/${legacyVehicleId}`, other.token, undefined, 'GET', 403);
        await request(`/vehicles/${legacyVehicleId}`, admin.token, undefined, 'DELETE', 409);
      ...
      await t.test('generates monthly bills idempotently and scopes ownership', async () => {
        ...
        await request('/payments/monthly?month=2026-99', guardian.token, undefined, 'GET', 400);
        await request('/admin/bills/generate', admin.token, { month: '2099-01' }, 'POST', 400);
      });
      const bill = (await request<Bill[]>('/payments/monthly', guardian.token))[0];
      const paymentInput = {
        billId: bill.id,
        method: 'BKASH',
      ...
      await t.test('supports custom QR accounts and private editable evidence', async () => {
        ...
        await request(
          '/admin/payment-accounts/BANK',
          admin.token,
          { ...account, imageUrl: 'data:image/svg+xml;base64,AAAA' },
          'PUT',
          400,
        );
        ...
        assert.deepEqual(
          accounts.find((item) => item.method === 'BANK'),
          { method: 'BANK', ...account },
        );
        const input = {
          ...paymentInput,
        ...
        await request(
          ...
          guardian.token,
          { ...input, evidenceImageUrl: 'invalid' },
          'POST',
          400,
        );
        const submission = await request<any>(
          '/payments/submissions',
          guardian.token,
          { ...input, evidenceImageUrl: image, transactionInfo: 'Paid at school branch' },
          'POST',
          201,
        );
        ...
        await request(
          endpoint,
          admin.token,
          { transactionInfo: 'No guardian permission' },
          'PATCH',
          403,
        );
        ...
        const updated = await request<any>(
          endpoint,
          guardian.token,
          { transactionId: 'BANK/2026-09.123', transactionInfo: 'Branch reference added' },
          'PATCH',
        );
        ...
        assert.equal(
          adminHistory.find((item) => item.id === submission.id).evidenceImageUrl,
          image,
        );
        await request(
          '/admin/payment-accounts/BANK',
          admin.token,
          { ...account, name: 'Renamed Bank' },
          'PUT',
          204,
        );
        assert.equal(
          (await request<any[]>('/payments/submissions', guardian.token))[0].methodName,
          'School Bank',
        );
        await request(
          `/admin/payments/${submission.id}/decision`,
          admin.token,
          { decision: 'REJECTED', note: 'Please correct the transfer' },
          'PATCH',
        );
        await request(endpoint, guardian.token, { transactionInfo: 'Too late' }, 'PATCH', 409);
      });
      let payment: PaymentSubmission;
      await t.test('validates manual submission and snapshots the receiver', async () => {
        await request('/payments/submissions', guardian.token, paymentInput, 'POST', 400);
        await request(
        ...
        await request('/payments/submissions', other.token, paymentInput, 'POST', 404);
        await request(
          '/payments/submissions',
          guardian.token,
        ...
        assert.equal(
          (await request<Bill[]>('/payments/monthly', guardian.token))[0].status,
          'UNPAID',
        );
        await request('/payments/submissions', guardian.token, paymentInput, 'POST', 409);
        await request(
        ...
        assert.equal(
          (await request<PaymentSubmission[]>('/payments/submissions', guardian.token))[0]
            .recipientNumber,
          '01700000001',
        );
      });
      await t.test('rejects with a reason and allows corrected resubmission', async () => {
        await request(
          `/admin/payments/${payment!.id}/decision`,
          admin.token,
          { decision: 'REJECTED' },
      ...
      await t.test('blocks a reused transaction ID on a different bill', async () => {
        ...
        const unpaid = (await request<Bill[]>('/payments/monthly', guardian.token)).find(
      ...
      await t.test('rolls back all payment writes if notification persistence fails', async () => {
        const db = app.get(DatabaseService);
        const unpaid = (await request<Bill[]>('/payments/monthly', guardian.token)).find(
          (item) => item.status === 'UNPAID',
        )!;
      ...
      await t.test('complaints resolve and stop approvals revoke live socket access', async () => {
        ...
        const gateway = new RealtimeGateway(app.get(AuthService), app.get(AccessService));
        let deliveries = 0;
        ...
        let invalidDisconnected = false;
        const adminClient = {
          handshake: { auth: { token: admin.token } },
          emit: () => {
            adminDeliveries++;
          },
          disconnect: () => assert.fail('valid admin should remain connected'),
        };
        const otherClient = {
          handshake: { auth: { token: other.token } },
          emit: () => {
            otherDeliveries++;
          },
          disconnect: () => assert.fail('unassigned guardian should remain connected'),
        };
        const invalidClient = {
          handshake: { auth: { token: 'f'.repeat(64) } },
          emit: () => assert.fail('unknown session must never receive locations'),
          disconnect: () => {
            invalidDisconnected = true;
          },
        };
        ...
        assert.equal(adminDeliveries, 1);
        assert.equal(otherDeliveries, 0);
        assert.equal(invalidDisconnected, true);
        for (const active of await request<Identified[]>('/subscriptions', guardian.token)) {
        ...
        await gateway.publishLocation({ imei: '868720065798377' });
        assert.equal(deliveries, 1, 'already connected sockets must lose tracking access');
        await request('/locations/868720065798377', guardian.token, undefined, 'GET', 403);
        assert.deepEqual(await request('/vehicles', guardian.token), {
          vehicles: [],
        });
        ...
        assert.equal(adminDeliveries, 3);
        assert.equal(otherDeliveries, 0);
        await request('/auth/me', guardian.token, undefined, 'GET', 401);
      });
      await t.test('does not persist raw session tokens or passwords', async () => {
        const db = app.get(DatabaseService);
        assert.equal(
          await db.get('SELECT "tokenHash" FROM sessions WHERE "tokenHash" = $1', admin.token),
          undefined,
        );
        assert.notEqual(
          (await db.get('SELECT "passwordHash" FROM users WHERE id = $1', admin.user.id))
            .passwordHash,
