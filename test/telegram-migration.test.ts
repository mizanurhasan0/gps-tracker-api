...
test('database migration 8 creates Telegram tables and enforces delivery idempotency', { skip: !databaseUrl }, async (t) => {
  ...
  await db.onModuleInit();
  assert.equal(
    Number((await db.all<{ count: string }>('SELECT count(*)::text AS count FROM app_migrations WHERE version = 8'))[0].count),
    1,
  );
});
